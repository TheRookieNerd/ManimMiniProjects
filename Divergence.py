...
class DivComponents(MovingCameraScene):
	...
	def SumTwoFields(self):
		...
		parent3=VectorField(functionrandom)
		parent3b=VectorField(functionrandom, opacity=.25)
		xcomp=VGroup()
		ycomp=VGroup()
	...
	def thumbnail(self):
		...
		circle=Line(LEFT*4,RIGHT*4).move_to(UP*4)
		arrow1=Arrow(np.array([0,4,0]),np.array([0,0,0])).set_color(RED)
		arrow2=Arrow(np.array([0,-1,0]),np.array([0,-5,0])).set_color(GREEN)
		self.play(ShowCreation(arrow1), )
		text=TextMobject("The Rookie Nerds").scale(2).move_to(np.array([0,-.5,0]))
		self.play(Write(text), run_time=2)
		self.wait()
		text1=TextMobject("Link in the comments :)").scale(1.5).move_to(np.array([0,-6,0]))
		grp=VGroup(arrow1,arrow2, text,text1)
		self.play(
			grp.shift, UP*4
			)
		self.wait()
...
class Curl(DivComponents):
	CONFIG={
		"curlfield_sub_kwargs":{
			"x_max": .5,
			"x_min": -.5,
			"y_max": .5,
			"y_min": -.5,
	...
	def curl(self):
		grid=NumberPlane()
		self.add(grid)
		curlfield=VectorField(functioncurl)
		curlfield_sub1=VectorField(functioncurledit, **self.curlfield_sub_kwargs)
		curlfield_sub2=VectorField(functioncurl, opacity=.25)
		self.add(curlfield)
		self.wait()
		self.play(
			ReplacementTransform(curlfield, curlfield_sub2),
			FadeIn(curlfield_sub2),
			self.camera_frame.scale, .25,
			run_time=3
			)
		self.i=.0001
		circle=Circle(radius=.05, color=YELLOW).move_to(ORIGIN)
		circle_text=TextMobject("(0,0)", color=YELLOW).move_to(circle.get_corner(DR)+RIGHT*.15+DOWN*.05).scale(.25)
		self.play(ShowCreation(circle))
		...
		P_list=[]
		for vect in ORIGIN+LEFT,ORIGIN+LEFT*.01,ORIGIN+RIGHT*.01 ,ORIGIN+RIGHT:
			self.play(
				ApplyMethod(demo_dot.move_to, vect,),
				#rate_func= linear,
				run_time=2)
